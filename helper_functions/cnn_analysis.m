...
augimdsTest = augmentedImageDatastore(imageSizeColor,testing_images,testing_labels, ...
    'DataAugmentation',imageAugmenter, ...
    'OutputSizeMode','randcrop');
...
% augmented_color_cnn_predict = cnn(
...
%         if uint8(color_cnn_predict(i)) == uint8(validation(i))
%             total = total +1;
%         end
%     end
% 
...
% cm = confusionchart(label_encoder(uint8(testing_labels)),label_encoder(uint8(color_cnn_predict)));
% cm.Title = 'Confusion Matrix for Color Validation Data';
